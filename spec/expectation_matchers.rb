describe 'Expectation Matchers' do 
  ...
  describe 'thruthiness matchers' do 
    ...
    it 'will match thruthy/falsy' do 
      expect(1 < 2).to be_truthy 
      expect(1 > 2).to be_falsy

      expect('foo').to be_truthy      #any value counts as truthy
      expect(nil).to be_falsy         #nil counts as falsy 
      expect(0).not_to be_falsy       #but 0 is still not falsy enough
    end 
  ...
  describe 'collection matchers' do 
    it 'will match arrays' do
      ...
      expect(array).not_to contain_exactly(1,2) #but uses individual arguments
    end
    ...
    it "will match strings" do 
      ...
      expect(string).to include('so', 'string')
  ...
  describe 'other useful matchers' do
    ...
    it 'will match object types' do 
      ...
      expect('test').to be_instance_of(String) #alias of #be_instance_of 
  ...
  describe 'predicate matchers' do 
    it 'will match be_* to custom methods ending in ?' do 
      #drops the "be_", adds "?" to end, calls method on object
    ...
    it 'will match have_* to custom methods like has_*?' do 
      #changes "have_" to "has_", adds "?" at the end, calls method on object
      ...
      expect(customer.has_pending_order?).to be true #same thing
