describe 'Doubles' do 
  ...
  it "allows stubbing multiple methods with hash syntax" do
    dbl = double("Person")
    allow(dbl).to receive_messages(:full_name => 'Micky Mouse', :initials => 'MM')
    expect(dbl.full_name).to eq("Micky Mouse")
    expect(dbl.initials).to eq("MM")
  end 

  it "allows stubbing with a hash argument to #double" do 
    dbl = double("Person", :full_name => "Mickey Mouse", :initials => "MM")
    expect(dbl.full_name).to eq("Mickey Mouse")
    expect(dbl.initials).to eq('MM')
  end 
  ...
  it "allows stubbing multiple responses with #and_return" do 
    ...
    expect(die.roll).to eq(6)  # continutes returning last number  
  end
  ...
  context 'with partial test doubles' do 
    ...
    it "allows stubbing class methods on Ruby classes" do 
      ...
      expect(Time.now.to_s).to eq('2010-01-01 12:00:00 +0100')
  ...
  context 'with argument contraints' do 
    ...
    it 'allows constraining in some arguments' do 
      dbl = double("Customer List")
  ...
  context 'with message count constraints' do
    ...
    it 'allows using at_least/at_most' do
      ...
      post.like(:user => 'Barney')
      post.like(:user => 'Marshall')
      post.like(:user => 'Ted')
      post.like(:user => 'Lily')      
    end 
  end 
  ...
  context 'with spying abilities' do 
    ...
    it 'can expect a call after it has been received' do 
      dbl = spy("Chant")
      allow(dbl).to receive(:hey!).and_return("Ho!")
      dbl.hey!
    ...
    it 'can expect any message already sent to a declared spy' do 
      ...
      # Notice that we don't use a stub :send_invoice
